...
export default function LivePreviewExample({ setform, form }) {
  const {
    acceptedFiles,
    isDragActive,
    isDragAccept,
  ...
  useEffect(() => {
    const runAction = async () => {
      if (acceptedFiles?.length) {
        const res = await getBase64(acceptedFiles.map((file) => file)[0]);
        setform({
          ...form,
          connectorImage: res
        });
        // setform(('connectorImage', res));
      } else {
        setform({
          ...form,
          connectorImage: null
        });
        // setform('connectorImage', null);
      }
    };
    runAction();
  }, [acceptedFiles]);
